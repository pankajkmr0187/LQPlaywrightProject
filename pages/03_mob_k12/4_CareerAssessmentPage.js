...
export class CareerAssessmentPage extends BasePage {
  ...
  async verifyCareerAssessmentPageContent() {
    ...
    // Delete old Career Assessment files
    console.log("🧹 Cleaning old Career Assessment files...");
    if (fs.existsSync(csvDir)) {
      const files = fs.readdirSync(csvDir);
      files.forEach(file => {
        if (file.includes('4_CareerAssessmentPage')) {
    ...
    const csvPath = path.join(csvDir, `4_CareerAssessmentPage_${timestamp}.csv`);
        ...
        try {
          ...
          if (status >= 200 && status < 400) {
            ...
            await this.page.screenshot({ 
              path: path.join(screenshotDir, `4_CareerAssessmentPage_failed_${i + 1}_${timestamp}.png`),
              fullPage: true 
            });
          ...
          await this.page.screenshot({ 
            path: path.join(screenshotDir, `4_CareerAssessmentPage_error_${i + 1}_${timestamp}.png`),
            fullPage: true 
          });
    ...
    console.log(`\n📊 Career Assessment Links Report:`);
