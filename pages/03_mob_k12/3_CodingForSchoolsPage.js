...
export class CodingForSchoolsPage extends BasePage {
  ...
  async verifyCodingForSchoolsPageContent() {
    ...
    if (fs.existsSync(csvDir)) {
      ...
      files.forEach(file => {
        if (file.includes('3_CodingForSchoolsPage')) {
    ...
    const csvPath = path.join(csvDir, `3_CodingForSchoolsPage_${timestamp}.csv`);
        ...
        try {
          ...
          if (status >= 200 && status < 400) {
            ...
            await this.page.screenshot({ 
              path: path.join(screenshotDir, `3_CodingForSchoolsPage_failed_${i + 1}_${timestamp}.png`),
              fullPage: true 
            });
          ...
          await this.page.screenshot({ 
            path: path.join(screenshotDir, `3_CodingForSchoolsPage_error_${i + 1}_${timestamp}.png`),
            fullPage: true 
          });
    ...
    console.log(`\n📊 Coding for Schools Links Report:`);
